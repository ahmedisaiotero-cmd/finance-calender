 ...
 *
 * This is the only module allowed to call `prisma.activityEventRecord`.
...
export const prismaActivityEventStore: ActivityEventStore = {
  async insert(record) {
    try {
      const row = await prisma.activityEventRecord.create({
        data: {
          id: record.event.id,
      ...
      if (
        ...
        const existing =
          (await prisma.activityEventRecord.findFirst({
            where: {
              ...ownerWhere({
                userId: record.userId,
                workspaceId: record.workspaceId,
  ...
  async findById(owner, id) {
    const row = await prisma.activityEventRecord.findFirst({
  ...
  async list({ owner, limit, cursor }) {
    const rows = await prisma.activityEventRecord.findMany({
      where: {
        ...ownerWhere(owner),
        ...(cursor
          ? {
